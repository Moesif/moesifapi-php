...
class SendCurlTaskConsumer extends SendTaskConsumer {
    ...
    function __construct($applicationId, $options) {
        ...
        $this->_app_config_endpoint = $options['app_config_endpoint'];
        $this->_event_endpoint = $options['event_endpoint'];
    ...
    public function getAppConfig() {
        ...
        $url = $this->_protocol . "://" . $this->_host . $this->_app_config_endpoint;
        return $this->_execute_get_request($url);
    }

    /**
     * Write to the event endpoint.
     */
    public function createEvent($eventData) {

        $data = $this->_encode($eventData);
        $url = $this->_protocol . "://" . $this->_host . $this->_event_endpoint;
        return $this->_execute($url, $data);
      }
    ...
    protected function _execute($url, $data) {
        ...
        $headers = [
            ...
            'User-Agent: moesifapi-php/1.2.0'
        ];

        $rspheaders = [];

        curl_setopt($ch, CURLOPT_HEADERFUNCTION,
        function($curl, $header) use (&$rspheaders)
        {
            $len = strlen($header);
            ...
            $rspheaders[strtolower(trim($header[0]))][] = trim($header[1]);
        ...
        if (false === $response) {
            ...
            if (trim($response) == "1") {
                ...
                $res = $this->_handleError(0, $response);
                $body = substr($response, $header_size);
                return array($rspheaders, $body);
     ...
     * @return array($rspheaders, $body)
     */
    protected function _execute_get_request($url) {
        if ($this->_debug()) {
            $this->_log("Making blocking cURL call to $url");
        }
        $ch = curl_init();
    ...
    protected function _execute_forked($url, $data) {
        ...
        $exec = ' echo \'' . $data . '\' '.' | '.'curl -v -X POST -H "Content-Type: application/json" -H "User-Agent: moesif-laravel/1.4.9" -H "X-Moesif-Application-Id: ' . $applicationId .'" -d @- "' . $url . '"';
    ...
    /**
     * Read using a forked cURL process
     * @param $url
     * @param $data
     * @return object
     */
    protected function _execute_get_forked($url) {
        $applicationId = $this->_appId;
        if ($this->_debug()) {
            $this->_log("Making forked cURL call to $url");
        }
        $exec = 'curl -v -X GET -H "Content-Type: application/json" -H "User-Agent: moesif-laravel/1.4.9" -H "X-Moesif-Application-Id: ' . $applicationId .'"  "' . $url . '"';
